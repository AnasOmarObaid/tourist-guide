                              ...
                              <div class="tile cu-rounded">
                                    <div class="tile-body">
                                          <form class="form" method="POST"
                                                action="{{ route('dashboard.event.update', $event) }}"
                                                ...
                                                <div class="mb-3">
                                                      ...
                                                      <textarea class="form-control @error('description') is-invalid  @enderror" id="description" name="description"
                                                            placeholder="Description" rows="5">{{ old('description', $event->description) }}</textarea>
                                                      @error('description')
                                                            <small class="invalid-feedback"
                                                                  role="alert"><strong>{{ $message }}</strong></small>
                                                      @enderror
                                                ...
                                                <div class="mb-3">
                                                      ...
                                                      <select name="tags[]" class="form-select" id="tags_id"
                                                            multiple>
                                                            <option value="">Select Tags</option>
                                                            @foreach ($tags as $tag)
                                                                  <option value="{{ $tag->id }}"
                                                                        {{ collect(old('tags', $event->tags->pluck(['id'])))->contains($tag->id) ? 'selected' : '' }}>
                                                                        {{ $tag->name }}
                                                                  </option>
                                                            @endforeach
                                                      </select>
                                                ...
                                                <div class="mb-3 form-check">
                                                      <input class="form-check-input cu-rounded" type="checkbox"
                                                ...
                                                <div class="tile-footer text-center">
                                                      <button class="btn btn-primary w-100 mb-0 cu-rounded"
                                                            type="submit">
                                                            <i class="bi bi-check-circle-fill"></i>Edit Event
                                                      </button>
                                                </div>
                        ...
                        <div class="col">
                              ...
                              <div class="card event-card">
                                    <div class="event-image">
                                          <img id="previewImage" src="{{ $event->image_url }}" alt="Event">
                                          <div class="attendees users-stack">
                                                @foreach ($event->attendees_images as $image)
                                                      <img src="{{ asset($image) }}" alt="User1">
                                                @endforeach
                                          </div>
                                    </div>
                                    ...
                                    <div class="event-body">
                                          ...
                                          <div class="d-flex justify-content-between align-items-baseline">
                                                <div class="price-tag" id="price_preview">
                                                      ${{ old('price', $event->price) }}</div>
                                                <p class="status-badge {{ $event->status ? 'status-badge-success' : 'status-badge-danger' }}"
                                                      id="status_preview">
                                                      {{ $event->status ? 'Active' : 'Cancelled' }}</p>
                                          </div>

                                          <h5 class="card-title" id="title_preview">{{ old('title', $event->title) }}
                                          </h5>

                                          <p class="text-muted mb-1"><i class="bi bi-geo-alt-fill text-danger"></i>
                                                <span id="city_preview">{{ $event->city?->name }}</span> <span
                                                      id="venue_preview">{{ $event->venue }}</span>
                                          </p>

                                          <p class="text-secondary" id="description_preview">
                                                {{ old('description', $event->description) }}</p>
                                          ...
                                          <div>
                                                ...
                                                <div class="details mt-3"><span>👤</span>
                                                      <span
                                                            id="organizer_preview">{{ old('organizer', $event->organizer) }}</span>
                                                </div>

                                                <div class="d-flex justify-content-between align-items-center">
                                                      <div class="date">
                                                            <div class="details text-success mt-3"><span>📅</span>
                                                                  Start:
                                                                  <span id="start_at_preview">
                                                                        {{ old('start_at', $event->formatted_start_at) }}</span>
                                                            </div>
                                                            <div class="details text-danger"><span>📅</span> End:
                                                                  <span
                                                                        id="end_at_preview">{{ old('end_at', $event->formatted_end_at) }}</span>
                                                            </div>
                                                      </div>

                                                      <div class="dateStatus">
                                                            <p
                                                                  class="px-2 status-badge d-inline
